...
work_dir = r'E:\Herring Models\Daily'
...
for time in mytimes:
    ...
    for irow, icol in dike_transect_cells:
        ...
        temp_elev = np.array([dem_trans[irow,icol]])
...
for i in range(len(mytimes)):
    ...
    plt.savefig(work_dir+"\DikeSeaLevel\DikeSeaLevel0"+str(f"{i:02d}")+".png")
    plt.show()    
...
for i in range(len(mytimes)):
    ...
    plt.savefig(work_dir+"\DikeUpstrmSeaLevel\DikeUpstrmSeaLevel0"+str(f"{i:02d}")+".png")
    plt.show()
...
# Make a movie of near dike transect (.png files must be saved with Tools>Preferences>IPython Console>Graphics>Backend set to Automatic),
    # otherwise the files will not be the same size
os.chdir(r'E:\Herring\DikeSeaLevel')   
img = os.listdir(r'E:\Herring\DikeSeaLevel')
...
os.chdir(r'E:\Herring\DikeUpstrmSeaLevel')   
img = os.listdir(r'E:\Herring\DikeUpstrmSeaLevel')
