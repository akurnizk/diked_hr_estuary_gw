...
# https://coast.noaa.gov/slr/#/layer/sce/0/-7841612.480606288/5141767.357835255/11/satellite/26/0.8/2100/high/midAccretion
# https://tidesandcurrents.noaa.gov/map/index.html
# 2.5 m is int-high/high, 1.5 m is int/int-high, and 0.5 m is low/int-low
...
ax[0].set_xlabel('column #')
ax[0].set_ylabel('row #')
genu.quick_plot(np.ma.masked_array(dem_model,(mask_array_CheqWest!=1)&(mask_array_HR!=1)),vmin=0,vmax=20,ax=ax[0])
#genu.quick_plot(mask_array.astype(int),ax=ax[0]) # in row, column space
c1=ax[1].pcolormesh(cc[0],cc[1],(mask_array_CheqWest.astype(int))|(mask_array_HR.astype(int))) # in model coordinates
genu.plt.colorbar(c1,ax=ax[1],orientation='horizontal')
fig.gca().set_aspect('equal', adjustable='box')
ax[1].set_xlabel('X [m]')
ax[1].set_ylabel('Y [m]')
...
"""
mask_array_CheqWest_HR = mask_array_CheqWest + mask_array_HR
mask_array_CheqWest_HR[mask_array_CheqWest_HR==2]=1
...
# start at assumed steady-state levels (edit to include or disclude HR)
...
strt[0,(mask_array_CheqWest_HR==1)&(dem_model_nofill<=mean_sea_level_annual_west)] = mean_sea_level_annual_west # start with West surface water at mean Provincetown sea level
...
strt[0,(mask_array_CheqWest_HR==1)&(strt[0,:,:]==1)] = mean_sea_level_annual_west
...
strt[0,active_cells&(mask_array_CheqWest_HR!=1)&(mask_array_CheqEast!=1)&(ztop<0)] = 0 # start with surface water at zero elevation
...
# CHANGE FOR SLR
# """
# # With SLR - 0.5 meters SLR at CNR U/S sensor -> 0.3 meters SLR at High Toss (use slope to adjust mean_HR_levels)
# CNRUS_to_HighToss_km = (((HighToss_UTM[0]-CNRUS_UTM[0])**2+(HighToss_UTM[1]-CNRUS_UTM[1])**2)**0.5)/1000
# # mean_HR_levels = mean_HR_levels + 0.5 - 0.2/CNRUS_to_HighToss_km*disp_CNRUS_sensor # 0.5 meters SLR results in 0.3 meters rise at High Toss (Int-Low)
# # mean_HR_levels = mean_HR_levels + 1.5 - 0.9/CNRUS_to_HighToss_km*disp_CNRUS_sensor # 1.5 meters SLR (Int-High)
# mean_HR_levels = mean_HR_levels + 2.5 - 1.5/CNRUS_to_HighToss_km*disp_CNRUS_sensor # 2.5 meters SLR (Extreme)
...
for i in range(nper): # there are 12 stress periods (nper)
    ...
    stage_cells_west = (mask_array_CheqWest_HR==1) & ~np.isnan(dem_model_nofill) & (dem_model_nofill <= sea_level_west)
    ...
    stage_cells = stage_cells_west + stage_cells_east
    ...
    stagerows = np.append(stagerows_west, stagerows_east)
    stagecols = np.append(stagecols_west, stagecols_east)
    ...
    stage = np.append(stage_west_fwh, stage_east_fwh)
    conductance_stage = np.append(cond_west, cond_east)
...
"""
# head_ss_rest = head
# head_lowslr_rest = head
# head_intslr_rest = head
# head_highslr_rest = head

#%% Load previously saved head files

hds = bf.HeadFile(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present',modelname+'.hds'))
time = 0
head_ss_rest = hds.get_data(totim=hds.get_times()[time]) # steady-state head (0th time step)
head_ss_rest[head_ss_rest<-100] = np.nan

hds = bf.HeadFile(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR',modelname+'.hds'))
time = 0
head_lowslr_rest = hds.get_data(totim=hds.get_times()[time]) # steady-state head (0th time step)
head_lowslr_rest[head_lowslr_rest<-100] = np.nan

hds = bf.HeadFile(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR',modelname+'.hds'))
time = 0
head_intslr_rest = hds.get_data(totim=hds.get_times()[time]) # steady-state head (0th time step)
head_intslr_rest[head_intslr_rest<-100] = np.nan

hds = bf.HeadFile(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR',modelname+'.hds'))
time = 0
head_highslr_rest = hds.get_data(totim=hds.get_times()[time]) # steady-state head (0th time step)
head_highslr_rest[head_highslr_rest<-100] = np.nan
...
dx,dy,gt = rastu.get_rast_info(demfname)
projwkt = rastu.load_grid_prj(demfname)
fname = os.path.join(work_dir, 'Cheq_Head_Elevation_Topo_HighSLR_restored.tif')
...
# Elevs in NAVD88
head_Phrag_Wetland_ss = head_ss_rest[0,np.around(Phrag_Wetland_colrow[1]).astype(int),np.around(Phrag_Wetland_colrow[0]).astype(int)]
...
head_Wet_Shrub_ss = head_ss_rest[0,np.around(Wet_Shrub_colrow[1]).astype(int),np.around(Wet_Shrub_colrow[0]).astype(int)]
...
head_Dry_Forest_ss = head_ss_rest[0,np.around(Dry_Forest_colrow[1]).astype(int),np.around(Dry_Forest_colrow[0]).astype(int)]
...
head_DogLeg_ss = head_ss_rest[0,np.around(DogLeg_colrow[1]).astype(int),np.around(DogLeg_colrow[0]).astype(int)]
elev_DogLeg_DEM = dem_model_nofill[np.around(DogLeg_colrow[1]).astype(int),np.around(DogLeg_colrow[0]).astype(int)]
head_HighToss_ss = head_ss_rest[0,np.around(HighToss_colrow[1]).astype(int),np.around(HighToss_colrow[0]).astype(int)]
elev_HighToss_DEM = dem_model_nofill[np.around(HighToss_colrow[1]).astype(int),np.around(HighToss_colrow[0]).astype(int)]

head_CNRUS_ss = head_ss_rest[0,np.around(CNRUS_colrow[1]).astype(int),np.around(CNRUS_colrow[0]).astype(int)]
...
genu.quick_plot(head_ss_rest[0],vmin=-0.04,vmax=2.0,ax=ax[0])
...
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_cells_ss'), stage_cells_ss)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_cells_west_ss'), stage_cells_west_ss)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_west_ss'), stage_west_ss)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_cells_east_ss'), stage_cells_east_ss)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_east_ss'), stage_east_ss)

np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_cells_lowslr'), stage_cells_lowslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_cells_west_lowslr'), stage_cells_west_lowslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_west_lowslr'), stage_west_lowslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_cells_east_lowslr'), stage_cells_east_lowslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_east_lowslr'), stage_east_lowslr)

np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_cells_intslr'), stage_cells_intslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_cells_west_intslr'), stage_cells_west_intslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_west_intslr'), stage_west_intslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_cells_east_intslr'), stage_cells_east_intslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_east_intslr'), stage_east_intslr)

np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_cells_highslr'), stage_cells_highslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_cells_west_highslr'), stage_cells_west_highslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_west_highslr'), stage_west_highslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_cells_east_highslr'), stage_cells_east_highslr)
np.save(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_east_highslr'), stage_east_highslr)

#%% Load Stage Arrays

stage_cells_ss = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_cells_ss.npy'))
stage_cells_west_ss = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_cells_west_ss.npy')) 
stage_west_ss = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_west_ss.npy')) 
stage_cells_east_ss = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_cells_east_ss.npy')) 
stage_east_ss = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Present','stage_east_ss.npy')) 

stage_cells_lowslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_cells_lowslr.npy')) 
stage_cells_west_lowslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_cells_west_lowslr.npy')) 
stage_west_lowslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_west_lowslr.npy')) 
stage_cells_east_lowslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_cells_east_lowslr.npy')) 
stage_east_lowslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-Low SLR','stage_east_lowslr.npy')) 

stage_cells_intslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_cells_intslr.npy')) 
stage_cells_west_intslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_cells_west_intslr.npy')) 
stage_west_intslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_west_intslr.npy')) 
stage_cells_east_intslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_cells_east_intslr.npy')) 
stage_east_intslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Int-High SLR','stage_east_intslr.npy')) 

stage_cells_highslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_cells_highslr.npy')) 
stage_cells_west_highslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_cells_west_highslr.npy')) 
stage_west_highslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_west_highslr.npy')) 
stage_cells_east_highslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_cells_east_highslr.npy')) 
stage_east_highslr = np.load(os.path.join('E:\Herring Models\Seasonal\Final_SS_v5\Restored\Extreme SLR','stage_east_highslr.npy')) 

#%% SS Adjustments

# Replace Stage Cells (Remove FW Equivalent head_ss_rest)
head_ss_rest_toplayer = head_ss_rest[0]
head_ss_rest_toplayer[stage_cells_west_ss==True] = stage_west_ss
head_ss_rest_toplayer[stage_cells_east_ss==True] = stage_east_ss

# Depth to W.T.
land_cells_ss = (mask_array==1)&~np.isnan(ztop)&(~stage_cells_ss)
depth_to_wt_ss = (dem_model_nofill-head_ss_rest_toplayer) #*land_cells_ss.astype(int)
# depth_to_wt_ss[depth_to_wt_ss<=0] = np.nan
...
plt.xlabel('Lx')
plt.ylabel('Ly')
head_ss_restplot = plt.contour(head_ss_rest[0])
plt.colorbar(head_ss_restplot, label='head_ss_rest (m)') # plots head_ss_rest_sss as contours 
# plt.colorbar.set_label('head_ss_rest_sss')
plt.savefig(os.path.join(work_dir,'CheqModel2a_ss_rest.png'))

# mytimes = np.arange(0, nper, 1) # for plotting nper-1 graphs
# for time in mytimes: 
#     head_ss_rest_ss = hds.get_data(totim=hds.get_times()[time]) # steady-state head_ss_rest_ss (0th time step)
#     head_ss_rest_ss[head_ss_rest_ss<-100] = np.nan
#     Cheqhead_ss_rest_ssplot = genu.quick_plot(head_ss_rest_ss, vmin=0.0, vmax = 2.5) # plots head_ss_rest_sss with color gradient
#     Cheqhead_ss_rest_sss = Cheqhead_ss_rest_ssplot.get_figure()
#     Cheqhead_ss_rest_sss.suptitle(str(time)+" hrs") # how does one label the figure?
#     Cheqhead_ss_rest_sss.savefig(os.path.join(work_dir,"Cheqhead_ss_rest_sssCheqhead_ss_rest_sss0"+str(f"{time:02d}")+".png"))

#%% Low SLR Adjustments

# Replace Stage Cells (Remove FW Equivalent Head)
head_lowslr_rest_toplayer = head_lowslr_rest[0]
head_lowslr_rest_toplayer[stage_cells_west_lowslr==True] = stage_west_lowslr
head_lowslr_rest_toplayer[stage_cells_east_lowslr==True] = stage_east_lowslr

# Depth to W.T.
land_cells_lowslr = (mask_array==1)&~np.isnan(ztop)&(~stage_cells_lowslr)
depth_to_wt_lowslr = (dem_model_nofill-head_lowslr_rest_toplayer) #*land_cells_lowslr.astype(int)
...
plt.xlabel('Lx')
plt.ylabel('Ly')
head_lowslr_restplot = plt.contour(head_lowslr_rest[0])
plt.colorbar(head_lowslr_restplot, label='head_lowslr_rest (m)') # plots head_lowslr_rests as contours 
# plt.colorbar.set_label('head_lowslr_rests')
plt.savefig(os.path.join(work_dir,'CheqModel2a_lowslr_rest.png'))
...
# Replace Stage Cells (Remove FW Equivalent Head)
head_intslr_rest_toplayer = head_intslr_rest[0]
head_intslr_rest_toplayer[stage_cells_west_intslr==True] = stage_west_intslr
head_intslr_rest_toplayer[stage_cells_east_intslr==True] = stage_east_intslr

# Depth to W.T.
land_cells_intslr = (mask_array==1)&~np.isnan(ztop)&(~stage_cells_intslr)
depth_to_wt_intslr = (dem_model_nofill-head_intslr_rest_toplayer) #*land_cells_intslr.astype(int)
...
plt.xlabel('Lx')
plt.ylabel('Ly')
head_intslr_restplot = plt.contour(head_intslr_rest[0])
plt.colorbar(head_intslr_restplot, label='head_intslr_rest (m)') # plots head_intslr_rests as contours 
# plt.colorbar.set_label('head_intslr_rests')
plt.savefig(os.path.join(work_dir,'CheqModel2a_intslr_rest.png'))
...
# Replace Stage Cells (Remove FW Equivalent Head)
head_highslr_rest_toplayer = head_highslr_rest[0]
head_highslr_rest_toplayer[stage_cells_west_highslr==True] = stage_west_highslr
head_highslr_rest_toplayer[stage_cells_east_highslr==True] = stage_east_highslr

# Depth to W.T.
land_cells_highslr = (mask_array==1)&~np.isnan(ztop)&(~stage_cells_highslr)
depth_to_wt_highslr = (dem_model_nofill-head_highslr_rest_toplayer) #*land_cells_highslr.astype(int)
...
plt.xlabel('Lx')
plt.ylabel('Ly')
head_highslr_restplot = plt.contour(head_highslr_rest[0])
plt.colorbar(head_highslr_restplot, label='head_highslr_rest (m)') # plots head_highslr_rests as contours 
# plt.colorbar.set_label('head_highslr_rests')
plt.savefig(os.path.join(work_dir,'CheqModel2a_highslr_rest.png'))
...
# Note, consruction documents use MLW as the datum. Use https://vdatum.noaa.gov/vdatumweb/vdatumweb?a=165504620191119
# to convert to NAVD 88 (my coordinates)

mean_HR_levels_present = np.zeros_like(disp_CNRUS_sensor)
mean_HR_levels_present[:,:] = np.array([mean_sea_level_monthly_west.mean()])

mean_HR_level_CNRUS = mean_HR_levels_present[int(np.around(CNRUS_colrow[1])),int(np.around(CNRUS_colrow[0]))]
mean_HR_level_DogLeg = mean_HR_levels_present[int(np.around(DogLeg_colrow[1])),int(np.around(DogLeg_colrow[0]))]
mean_HR_level_HighToss = mean_HR_levels_present[int(np.around(HighToss_colrow[1])),int(np.around(HighToss_colrow[0]))]
...
mean_HR_levels_intslr[:,:] = np.array([mean_sea_level_monthly_west.mean()]) + 1.5

mean_HR_levels_highslr = mean_HR_levels_present.copy()
mean_HR_levels_highslr[:,:] = np.array([mean_sea_level_monthly_west.mean()]) + 2.5
...
plt.figure()
nonzero_depth_to_wt_ss_HR = np.count_nonzero(depth_to_wt_ss[mask_array_SLAMM!=0])
plt.hist(depth_to_wt_ss[mask_array_SLAMM!=0],range=(np.nanmin(depth_to_wt_highslr[mask_array_SLAMM!=0]),np.nanmax(depth_to_wt_ss[mask_array_SLAMM!=0])),label='Present Day')
nonzero_depth_to_wt_lowslr_HR = np.count_nonzero(depth_to_wt_lowslr[mask_array_SLAMM!=0])
plt.hist(depth_to_wt_lowslr[mask_array_SLAMM!=0],range=(np.nanmin(depth_to_wt_highslr[mask_array_SLAMM!=0]),np.nanmax(depth_to_wt_ss[mask_array_SLAMM!=0])),label='0.5 Meter SLR (m)')
nonzero_depth_to_wt_intslr_HR = np.count_nonzero(depth_to_wt_intslr[mask_array_SLAMM!=0])
plt.hist(depth_to_wt_intslr[mask_array_SLAMM!=0],range=(np.nanmin(depth_to_wt_highslr[mask_array_SLAMM!=0]),np.nanmax(depth_to_wt_ss[mask_array_SLAMM!=0])),label='1.5 Meter SLR (m)')
nonzero_depth_to_wt_highslr_HR = np.count_nonzero(depth_to_wt_highslr[mask_array_SLAMM!=0])
plt.hist(depth_to_wt_highslr[mask_array_SLAMM!=0],range=(np.nanmin(depth_to_wt_highslr[mask_array_SLAMM!=0]),np.nanmax(depth_to_wt_ss[mask_array_SLAMM!=0])),label='2.5 Meter SLR (m)')
plt.legend()
plt.xlabel('depth to wt')
plt.ylabel('# occurences')
...
sw_levels_present = head_ss_rest[0].copy()
...
cbar.set_label("Modeled Surface Water Depth (Restored) [m]")

dx,dy,gt = rastu.get_rast_info(demfname_nofill)
projwkt = rastu.load_grid_prj(demfname_nofill)
fname_sw_present = os.path.join(work_dir,'Cheq_modelSW_restored.tif')
...
head_ss_sw_masked = head_ss_rest[0].copy()
head_ss_sw_masked[head_ss_sw_masked>dem_model_nofill] = np.nan

emergent_gw_lowslr = head_lowslr_rest[0].copy()
...
emergent_gw_intslr = head_intslr_rest[0].copy()
...
emergent_gw_highslr = head_highslr_rest[0].copy()
...
dx,dy,gt = rastu.get_rast_info(demfname_nofill)
projwkt = rastu.load_grid_prj(demfname_nofill)
fname_lowslr = os.path.join(work_dir,'CheqEmergentGW_lowslr_rest.tif')
rastu.write_gdaltif(fname_lowslr,cc_proj[0],cc_proj[1],emergent_gw_lowslr,proj_wkt=projwkt)
fname_intslr = os.path.join(work_dir,'CheqEmergentGW_intslr_rest.tif')
rastu.write_gdaltif(fname_intslr,cc_proj[0],cc_proj[1],emergent_gw_intslr,proj_wkt=projwkt)
fname_highslr = os.path.join(work_dir,'CheqEmergentGW_highslr_rest.tif')
...
depth_to_wt_present_hydrofill = dem_model-head_ss_rest[0]
depth_to_wt_present_hydrofill[depth_to_wt_present_hydrofill<=0.5] = np.nan

head_range_lowSLR = head_lowslr_rest[0] - head_ss_rest[0]
head_range_intSLR = head_intslr_rest[0] - head_ss_rest[0]
head_range_highSLR = head_highslr_rest[0] - head_ss_rest[0]
...
# Plots of head ranges, vmax at 0.5m SLR
...
# cbar.set_label("$\Delta$WT [m]")
# fig.suptitle('Restored Conditions')
...
head_range_lowSLR_SWmasked[(head_lowslr_rest[0]-dem_model_nofill)>0] = np.nan
head_range_intSLR_SWmasked[(head_intslr_rest[0]-dem_model_nofill)>0] = np.nan
head_range_highSLR_SWmasked[(head_highslr_rest[0]-dem_model_nofill)>0] = np.nan
...
genu.quick_plot(dem_model_nofill[None,:]-np.vstack([head_lowslr_rest[0][None,:],head_ss_rest[0][None,:]]),vmin=0,vmax=1)
genu.quick_plot(dem_model_nofill[None,:]-np.vstack([head_intslr_rest[0][None,:],head_ss_rest[0][None,:]]),vmin=0,vmax=1)
genu.quick_plot(dem_model_nofill[None,:]-np.vstack([head_highslr_rest[0][None,:],head_ss_rest[0][None,:]]),vmin=0,vmax=1)
fig,ax = genu.plt.subplots(1,3)
ax[0].set_xlabel('column #')
ax[0].set_ylabel('row #')
ax[0].title.set_text('0.5 Meter SLR (m)')
...
dx,dy,gt = rastu.get_rast_info(demfname_nofill)
projwkt = rastu.load_grid_prj(demfname_nofill)
fname_lowslr = os.path.join(work_dir,'CheqSLRInfluence_lowslr_rest.tif')
rastu.write_gdaltif(fname_lowslr,cc_proj[0],cc_proj[1],SLRrange_low,proj_wkt=projwkt)
fname_intslr = os.path.join(work_dir,'CheqSLRInfluence_intslr_rest.tif')
rastu.write_gdaltif(fname_intslr,cc_proj[0],cc_proj[1],SLRrange_int,proj_wkt=projwkt)
fname_highslr = os.path.join(work_dir,'CheqSLRInfluence_highslr_rest.tif')
rastu.write_gdaltif(fname_highslr,cc_proj[0],cc_proj[1],SLRrange_high,proj_wkt=projwkt)
