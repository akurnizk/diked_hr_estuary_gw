...
Q_total_dates = np.column_stack((x_datenum_dike,Q_total))
np.savetxt(os.path.join(data_dir, 'Discharge Data', 'Q_total_dikecalc.csv'), Q_total_dates, delimiter=',')
...
def NORMAL_D(YNORM,Q,CMAN,B0,S,S0):
    ...
    for i in range(999):
        ...
        DFDY = 1.6667*TOP(YNORM)*HR(YNORM)**0.6667 - 0.6667*HR(YNORM)**1.6667*C2
        YNEW = YNORM - FY/DFDY
...
s = 2 # channel lateral slope - adjust to improve AR estimate.

# Use average depths to calculate top width: optimize b0 to get top widths that match.
...
y0_act = np.linspace(elev_hightoss_avg,elev_cnrus_avg,40)-min_elevs # starting depths

wsp = y0_act + np.amin(elevs_interp,axis=1) # add depth to lowest point in channel
...
for xsec in range(nsec+1):
    b0_act_temp = top_widths[xsec]-2*y0_act[xsec]*s # solve for average bottom width (given y0_act means, top-width, and s)
    b0_act.append(b0_act_temp)
b0_act = np.array(b0_act)

# b0_act = 10 # bottom width old (single val)
cmn = 0.07 # manning's coefficient (use spaulding & grilli?)
# s0 = 0.001 # channel bottom slope - ignore. use dem to find elevs, take mean line fit

xsec_loc = np.cumsum(min_dist_dx)
xsec_loc = np.append([0],xsec_loc)
plt.plot(xsec_loc,b0_act,label='b0_act data')
"""
Should I find a linear function that fits the bottom widths for an average bottom width change?
"""
...
s0 = -z_elevs[0]
b0 = z_b0[0]*xsec_loc + z_b0[1]

q0 = 0.6 # initial steady state discharge (use monthly-averaged fw discharge values)
Z = z_elevs[0]*xsec_loc + z_elevs[1] # bottom elev array
y0 = np.linspace(elev_hightoss_avg,elev_cnrus_avg,40) - Z
...
while (T <= tlast) & (iflag == 0): # time loop, ntimes = tlast/dt
    ...
    print("High Toss AR = {0:3.2f} m^2".format(AR(Y)[0]))
    ITER = 0
    if (iprint == ip):
        ip = 0
    ...
    for L in range(1,1000):
        ...
        if (SUMM > tol):
            ...
            sf1 = make_sf(Y,V,cmn_squared)[:-1]
            sf2 = make_sf(Y,V,cmn_squared)[1:]
            term1 = dtx2*alpha*((V[1:]**2)*ARiP1 + grav*CENTR(Y)[1:]-
                                (V[:-1]**2)*ARi-grav*CENTR(Y)[:-1])
